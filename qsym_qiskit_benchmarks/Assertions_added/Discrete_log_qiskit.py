# Source: https://github.com/TRSasasusu/qiskit-quantum-zoo/blob/main/qqz/discrete_log.py
...
def discrete_log(a: int, b: int, p: int, show_hist: Optional[bool] = False, coef_t: Optional[int] = 1) -> int:
    ...
    # requires 0 <= b < p
    # requires gcd(a, p) == 1
    # requires exists s0 :: 0 <= s0 < p && pow_mod(a, s0, p) == b
    # ensures 0 <= s < p
    # ensures pow_mod(a, s, p) == b

    r = order_finding(x=a, N=p, show_hist=False)
    # assert r > 0
    # assert pow_mod(a, r, p) == 1
    # assert forall k :: 0 < k < r ==> pow_mod(a, k, p) != 1
    ...
    # assert { first_register[0, t) : nor ↦ |0⟩ }
    # assert { second_register[0, t) : nor ↦ |0⟩ }
    # assert { third_register[0, 2*t) : nor ↦ |1⟩ }
    # assert { auxiliary_register_mid[0, t) : nor ↦ |0⟩ }
    # assert { auxiliary_register_end[0, 6*t-2) : nor ↦ |0⟩ }
    # assert { classical_register[0, 2*t) : 0}
    ...
    #assert { first_register[0,t) : en  ↦ ∑ k ∈ [0, 2^t) . 1/sqrt(2^t) | k ⟩ }
    #assert { second_register[0,t) : en  ↦ ∑ k ∈ [0, 2^t) . 1/sqrt(2^t) | k ⟩ }
